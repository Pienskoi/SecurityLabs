...
const { getUserToken, getUser, getAccessToken, createUser, refreshAccessToken, getCertificate, getTokenByCode } = require('./auth_service');
...
app.get('/login', async (req, res) => {
    res.redirect(`https://${domain}/authorize?response_type=code&client_id=${client_id}&redirect_uri=http://localhost:3000&scope=offline_access read:users read:current_user&audience=${audience}`);
});

app.get('/logout', (req, res) => {
    res.redirect(`https://${domain}/v2/logout?client_id=${client_id}&returnTo=http://localhost:3000/`);
});
...
app.post('/api/exchange_code', async (req, res) => {
    const { code } = req.body;
    const token = await getTokenByCode(domain, code, client_id, client_secret);
    console.log("Exchanged code for tokens: ", code);
